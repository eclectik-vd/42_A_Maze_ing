...
Principle:
+ Digs a path at random, moving in a random direction; when blocked, it backtracks until it finds a cell with an unexplored way out. [Details here](/doc/DFS_graph.md)

The maze is generated by **`MazeGenerator`** (`src/mazegen.py`):

1. `_generate_perfect_maze()` creates a perfect maze, `_apply_42_pattern()` incorporates the "42" pattern when the size allows it.
2. If `PERFECT=False`, `_make_imperfect()` removes all dead ends and makes the maze imperfect.
...
Principle:
+ Explores the graph level by level, processing all neighbors at distance _k_ before moving on to distance _k+1_. [Details here](/doc/BFS_graph.md)
